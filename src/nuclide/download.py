...
class Download:
    ...
    def get_ground_state(self, compromised=False):
        ...
        if compromised is True:
            ground_state["nuclide_id"] = ground_state["symbol"] + (
                ground_state["z"] + ground_state["n"]
            ).astype(str)
    ...
    def _download_single_nuclide(self, nuclide_id):
        """
        ...
        with requests.get(url, stream=True) as r:
            ...
            if len(r.content) > 2:
                ...
                logging.warning(
                    f"No data NUCLIDE-ID: {nuclide_id} found on API (probably not gamma)"
                )
        ...
        with requests.get(url, stream=True) as r:
            with open(self.ground_state_path, "wb") as f:
                for chunk in r.iter_content():
                    f.write(chunk)
        logging.info(
            f"Ground-State for all nuclides loaded and saved to {self.ground_state_path}"
        )
