...
sidebar = dbc.Col(
    [
        ...
        dcc.Dropdown(id="input-nuclides", multi=True, options=npi.API().unique_nuclides(),
                     value=list(load_config()["peakfinder"]["nuclides"])),

        html.Br(),
        html.H6("Tolerance", style={"color": "lightgrey"}),
        dcc.Input(load_config()["peakfinder"]["tolerance"], type="number", id="input-tolerance"),

        html.Br(),
        html.H6("Nuclide-Intensity", style={"color": "lightgrey"}),
        dcc.Input(load_config()["peakfinder"]["nuclide_intensity"], type="number", id="input-nuclide-intensity"),

        html.Br(),
        html.H6("Matching-Ration", style={"color": "lightgrey"}),
        dcc.Input(load_config()["peakfinder"]["matching_ratio"], type="number", id="input-matching-ratio"),

        html.Br(),
        html.H6("Prominence", style={"color": "lightgrey"}),
        dcc.Input(load_config()["peakfinder"]["prominence"], type="number", id="input-prominence"),
...
def get_nuclide_table(nuclide_intensity, max_energy, min_energy):
    data = npi.API().nuclides_by_intensity_and_min_max_energy(
        intensity=nuclide_intensity,
...
def update_combined_chart(file_id, tolerance, nuclide_intensity, matching_ratio, prominence, wlen, nuclides):
    fig = go.Figure()
    if file_id:
